...
(() => {
  ...
  function showSection(id) { const el = $(id); if (el) el.scrollIntoView({behavior:'smooth',block:'start'}); }
  ...
  function initAuth(){const open=()=>{openModal('authModal');state.member?null:authPanels('loginPanel');};$('accountBtn')?.addEventListener('click',open);$('accountChip')?.addEventListener('click',open);$('closeAuth')?.addEventListener('click',()=>closeModal('authModal'));$('showLoginPanel')?.addEventListener('click',()=>authPanels('loginPanel'));$('showRegisterPanel')?.addEventListener('click',()=>authPanels('registerPanel'));$('showForgotPanel')?.addEventListener('click',()=>authPanels('forgotPanel'));
...
(() => {
  ...
  function openStudioTab(page){$$('.studio-tab').forEach(x=>x.classList.toggle('active',x.dataset.page===page));$$('.studio-page').forEach(x=>x.classList.toggle('active',x.id===`page-${page}`));showSection('studio');if(page==='video')setTimeout(updateVideoCanvasSize,50);}
  ...
  ------------------------------------------------------------------- */
  const PLANS={trial:{label:'14일 이용권',price:6900,days:14},monthly:{label:'30일 이용권',price:14900,days:30},annual:{label:'365일 이용권',price:99000,days:365}};
