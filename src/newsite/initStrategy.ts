...
function upgradeShowContent(){const stage=$('#recommendStage').value,domain=$('#recommendDomain').value,r=upgradeRecommendation(stage,domain),s=stageData[stage];$('#contentGrid').innerHTML=[['BOOK',r.book,`${domain}을 이해하고 스스로에게 질문하는 읽기 경로`],['MUSIC',r.music,'감정을 정리하고 행동을 시작하도록 돕는 짧은 음악 경로'],['ACTION',r.action,`${s.name}에게 맞춘 오늘의 작은 실행`]].map(x=>`<article class="content-card"><span class="content-type">${x[0]}</span><h3>${escapeHtml(x[1])}</h3><p>${escapeHtml(x[2])}</p><strong>${x[0]==='BOOK'?'라이브러리에서 바로 읽기 →':'확인 → 실천 → 결과 기록'}</strong></article>`).join('');const bookCard=$('#contentGrid .content-card');if(bookCard){bookCard.style.cursor='pointer';bookCard.setAttribute('role','button');bookCard.title='전자책 바로 열기';bookCard.onclick=()=>window.dispatchEvent(new CustomEvent('dawon:open-book',{detail:{title:r.book}}))}}
