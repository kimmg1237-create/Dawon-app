// @ts-nocheck
/**
 * DAWON TRIPLE STRATEGY LAB — 페이지 동작 스크립트.
 * 제공된 HTML 시제품의 스크립트를 React 마운트 이후 1회 실행되는 초기화 함수로 이식했습니다.
...
const $=(s,r=document)=>{const n=r.querySelector(s);if(n)return n;const d=document.createElement('div');d.hidden=true;d.dataset.dawonStub='1';Object.defineProperty(d,'value',{get:()=>'',set:()=>{},configurable:true});Object.defineProperty(d,'elements',{get:()=>new Proxy({},{get:()=>d}),configurable:true});try{document.body?.appendChild(d)}catch{}return d};
const $$=(s,r=document)=>[...r.querySelectorAll(s)];
const form=$('#surveyForm'), pages=$$('.survey-page'), totalPages=Math.max(pages.length,1); let page=0, saveTimer;
const meta=[
 ...
 ['REVIEW','제출 전 검토','전체 응답을 확인하고 바람설계 보고서를 만듭니다.']
];
...
function showPage(i,scroll=true){if(!(form instanceof HTMLFormElement)||!pages.length)return;page=Math.max(0,Math.min(i,totalPages-1));pages.forEach((p,n)=>p.classList.toggle('active',n===page));$('#pageKicker').textContent=meta[page][0];$('#pageTitle').textContent=meta[page][1];$('#pageDesc').textContent=meta[page][2];const pct=Math.round(page/(totalPages-1)*100);$('#progressText').textContent=pct+'%';$('#progressBar').style.width=pct+'%';$('#prevBtn').style.visibility=page===0?'hidden':'visible';$('#nextBtn').hidden=page===totalPages-1;$('#submitBtn').hidden=page!==totalPages-1;renderSteps();if(page===totalPages-1)renderReview();if(scroll)$('#survey').scrollIntoView({behavior:'smooth',block:'start'})}
function getValues(){if(!(form instanceof HTMLFormElement))return{};const out={};new FormData(form).forEach((v,k)=>{if(out[k]!==undefined)out[k]=Array.isArray(out[k])?[...out[k],v]:[out[k],v];else out[k]=v});return out}
...
$('#voiceToggle').onclick=()=>{voiceOn=!voiceOn;$('#voiceToggle').textContent=voiceOn?'음성안내 끄기':'음성안내 켜기';if(voiceOn){speakSlide();toast('브라우저 음성안내를 시작합니다.')}else if('speechSynthesis' in window){speechSynthesis.cancel()}};if(aiSlides.length)startAi();
...
const upgradeDomains=['배움','진로와 일','마음','관계','건강','돈과 생활'];
...
const upgradeContents={
  '배움':{book:'《배운 것을 살아내다》',music:'집중과 작은 성취를 돕는 실천 음악',action:'배운 내용 한 가지를 오늘 10분 동안 직접 사용해보기'},
...
function upgradeApplyMode(key){
  ...
  $('#quickGuideTitle').textContent=key==='elementary'?'오늘 무엇을 해보고 싶나요?':key==='middle'?'지금 가장 마음에 걸리는 것은 무엇인가요?':'오늘 가장 바꾸고 싶은 것은 무엇인가요?';
  $('#quickGuideText').textContent=easy?'길게 쓰지 않아도 됩니다. 기분 한 가지와 오늘 해볼 작은 행동 한 가지만 적어보세요.':'현재 상황과 바람을 짧게 적고 오늘 실행할 가장 작은 행동을 정합니다.';
  $('#quickWishLabel').textContent=key==='elementary'?'오늘 더 잘하고 싶은 것은 무엇인가요?':'지금 가장 바꾸고 싶은 것은 무엇인가요?';
  $('#quickActionLabel').textContent=key==='elementary'?'오늘 내가 직접 해볼 작은 일':'오늘 할 수 있는 가장 작은 행동';
  $('#quickWish').placeholder=key==='elementary'?'예: 아침 준비를 스스로 하고 싶어요':'예: 마음을 회복하고 진로를 한 단계 확인하고 싶습니다';
  $('#quickAction').placeholder=key==='elementary'?'예: 자기 전에 가방을 10분 정리해요':'예: 오늘 저녁 8시에 10분 동안 자료 1개 확인하기';
}
...
function upgradeRecommendation(stage,domain){
  ...
  return {book:c.book,music:c.music,action:c.action,tool:stage==='elementary'?'그림·칭찬형 오늘의 미션카드':stage==='middle'?'사실·감정·욕구·요청 4줄 카드':stage==='high'?'진로가설 7일 검증표':stage==='college'?'경험 포트폴리오 5줄 기록':stage==='worker'?'업무 목적·완료기준 카드':stage==='midlife'?'건강·가족·재정·일 4영역 점검표':stage==='senior'?'건강·연락·경험기록 안전카드':'생활·직무·관계 7일 체크'};
}
...
$('#quickDesignForm')?.addEventListener('submit',e=>{e.preventDefault();const stage=$('#quickStage').value,subtype=$('#quickSubtype').value,wish=$('#quickWish').value.trim(),action=$('#quickAction').value.trim(),emotion=$('#quickEmotion').value.trim()||'아직 이름 붙이지 않음',time=$('#quickTime').value,domain=upgradeSelectedDomain;if(!wish||!action)return toast('바꾸고 싶은 것과 오늘 행동을 적어주세요.');const s=stageData[stage],r=upgradeRecommendation(stage,domain),support=s.support[0]||'도움을 요청할 사람 한 명을 정합니다.';const measure=`${time} 안에 실제로 시작하고, 완료·어려움·느낌 중 하나를 기록합니다.`;const week=`1일차 시작 → 2~5일차 반복과 수정 → 6일차 반응 확인 → 7일차 유지·수정·중단 결정`;$('#missionStage').textContent=`${s.name} · ${subtype} · ${domain}`;$('#missionTitle').textContent=wish;$('#missionIntro').textContent=`지금 감정은 “${emotion}”입니다. 정답보다 오늘의 작은 결과를 남깁니다.`;$('#missionAction').textContent=action;$('#missionMeasure').textContent=measure;$('#missionSupport').textContent=support;$('#missionWeek').textContent=week;$('#missionRecommend').innerHTML=`<b>맞춤 추천</b><p>책: ${escapeHtml(r.book)}<br>음악: ${escapeHtml(r.music)}<br>도구: ${escapeHtml(r.tool)}</p>`;$('#quickResult').classList.add('show');upgradeQuickText=`DAWON 3분 빠른 자기설계\n${new Date().toLocaleString('ko-KR')}\n\n[생애단계]\n${s.name} · ${subtype}\n\n[설계 분야]\n${domain}\n\n[지금 바람]\n${wish}\n\n[지금 감정]\n${emotion}\n\n[오늘 행동]\n${action}\n\n[완료 기준]\n${measure}\n\n[7일 방향]\n${week}\n\n[추천]\n${r.book}\n${r.music}\n${r.tool}`;storage.setItem(UPGRADE_QUICK_KEY,JSON.stringify({stage,subtype,domain,wish,action,emotion,time,savedAt:new Date().toISOString()}));window.dispatchEvent(new CustomEvent('dawon:quick-saved'));upgradeSyncStage(stage);upgradeSelectedDomain=domain;upgradeRenderStageRecommendation();toast('오늘의 실천카드를 만들었습니다.')});
...
function upgradeFillRecommendSelects(){const rs=$('#recommendStage'),rd=$('#recommendDomain');if(rs.dataset?.dawonStub||rd.dataset?.dawonStub)return;rs.innerHTML=Object.keys(stageData).map(k=>`<option value="${k}">${stageData[k].name}</option>`).join('');rd.innerHTML=upgradeDomains.map(x=>`<option>${x}</option>`).join('');rs.value=currentStage;rd.value=upgradeSelectedDomain}
function upgradeShowContent(){const grid=document.querySelector('#contentGrid');if(!grid||grid.dataset?.dawonStub)return;const stage=$('#recommendStage').value||currentStage||'elementary',domain=$('#recommendDomain').value||upgradeSelectedDomain,r=upgradeRecommendation(stage,domain),s=stageData[stage]||stageData.elementary;$('#contentGrid').innerHTML=[['BOOK',r.book,`${domain}을 이해하고 스스로에게 질문하는 읽기 경로`],['MUSIC',r.music,'감정을 정리하고 행동을 시작하도록 돕는 짧은 음악 경로'],['ACTION',r.action,`${s.name}에게 맞춘 오늘의 작은 실행`]].map(x=>`<article class="content-card"><span class="content-type">${x[0]}</span><h3>${escapeHtml(x[1])}</h3><p>${escapeHtml(x[2])}</p><strong>${x[0]==='BOOK'?'라이브러리에서 바로 읽기 →':'확인 → 실천 → 결과 기록'}</strong></article>`).join('');const bookCard=$('#contentGrid .content-card');if(bookCard){bookCard.style.cursor='pointer';bookCard.setAttribute('role','button');bookCard.title='전자책 바로 열기';bookCard.onclick=()=>window.dispatchEvent(new CustomEvent('dawon:open-book',{detail:{title:r.book}}))}}
if(document.querySelector('#recommendStage')&&!$('#recommendStage').dataset?.dawonStub){upgradeFillRecommendSelects();upgradeShowContent();$('#refreshRecommend')?.addEventListener('click',()=>{upgradeSelectedDomain=$('#recommendDomain').value;upgradeShowContent();toast('맞춤 콘텐츠를 다시 추천했습니다.')})}
...
// Accessibility: always keep large text; ignore legacy toggles.
document.body.classList.add('large-text');
document.body.classList.remove('high-contrast');
storage.setItem('dawonLargeText','true');
storage.removeItem('dawonHighContrast');
...
updateCounters();renderStage('elementary');upgradeSyncStage('elementary');upgradeRenderStageRecommendation();if(form instanceof HTMLFormElement&&pages.length)showPage(0,false);
}

function initStyleUpgrade() {
  const stageColors={
    elementary:'#f0c94d',
    middle:'#26a7c7',
    high:'#1976b8',
...
function initIntegratedStrategy() {
  'use strict';
  ...
  function renderLifeSignal(){
    ...
    else if(quick && trackerDays>=3 && reportCount===0){href='#survey';message='정밀 바람설문으로 경험과 다음 방향을 연결하세요.'}
  ...
  $('#downloadIntegratedGuide')?.addEventListener('click',()=>download('DAWON_3인전략모티프_통합전략.txt',`DAWON TRIPLE STRATEGY LAB
...
export function initStrategySite() {
  document.body.dataset.mood = 'warm';
  document.body.dataset.siteEdition = 'final-2026-07-19';
  const run = (name, fn) => {
    try {
      fn()
    } catch (error) {
      console.warn(`[dawon] ${name} init skipped:`, error)
    }
  }
  run('main', initMainSite)
  run('style', initStyleUpgrade)
  run('innovation', initInnovation)
  run('integrated', initIntegratedStrategy)
  run('proposal', initSmallProposal)
}
