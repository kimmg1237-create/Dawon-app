...
(() => {
  ...
  function renderAccess(){const access=window.DAWON_ACCESS_STATE;const active=Boolean(access.active);textSafe('libraryAccessPill',active?'전체 이용 가능':'무료 미리보기');textSafe('libraryAccessTitle',active?`${access.planName||'유료'} 이용권 · PDF 153개 작품관 이용 가능`:'전자책·만화·오디오북 미리보기 · 첫 가입 7일 무료 (아이디당 1회)');textSafe('libraryAccessDetail',active?(access.endsAt?`이용 만료: ${new Date(access.endsAt).toLocaleDateString('ko-KR')}`:'활성 이용권이 확인되었습니다.'):'미리보기는 누구나 이용할 수 있습니다. 가입 후 7일 무료 체험 기간에는 전자책 51·오디오북 낭독 PDF 51·만화책 51 전체가 열립니다.');textSafe('libraryAccessCta',active?'작품 검색하기':'이용권 보기');const cta=$('libraryAccessCta');if(cta){cta.href=active?'/library':'/subscribe';cta.textContent=active?'작품관 열기':'7일 무료 시작';}renderWorks();renderSubscription();}
