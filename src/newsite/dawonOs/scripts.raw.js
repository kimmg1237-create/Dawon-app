...
(() => {
  ...
  const t = (key, fallback) => {
    const v = window.__dawonI18n?.t?.(key, fallback);
  ...
  let batchValueRead = false;
  let sectionNavObserver = null;
  function beginPageBindings() {
    window.__dawonOsPageAbort?.abort();
    const ac = new AbortController();
    window.__dawonOsPageAbort = ac;
    pageAbortSignal = ac.signal;
    pageAbortSignal.addEventListener('abort', () => {
      sectionNavObserver?.disconnect();
      sectionNavObserver = null;
    });
  }
  function bindEl(el, type, fn) {
    if (!el || !pageAbortSignal) return;
    el.addEventListener(type, fn, { signal: pageAbortSignal });
  }
  function bindAll(nodes, type, fn) {
    nodes.forEach((el) => bindEl(el, type, fn));
  }
  ...
  function withEmo(value = '') {
    return escapeHtml(value).replace(/\[\[emo:?(\d{2})\]\]/g,'<img class="dawon-emo" src="/emoticons/$1.png" alt="">');
  }
  function flushEmoField(el) {
    if (el instanceof HTMLInputElement || el instanceof HTMLTextAreaElement) {
      window.__dawonFlushEmoticonField?.(el);
    }
  }
  function flushEmoFields() {
    window.__dawonFlushEmoticons?.();
  }
  ...
  function value(id, next) {
    ...
    if (!batchValueRead) flushEmoField(el);
    return el.value || '';
  }
  ...
  function localeTag() {
    const lang = window.__dawonI18n?.lang || 'ko';
    return lang === 'en' ? 'en-US' : lang === 'ja' ? 'ja-JP' : lang === 'zh' ? 'zh-CN' : 'ko-KR';
  }
  function dateLabel(key) {
    const [y, m, d] = String(key).split('-').map(Number);
    if (!y || !m || !d) return key;
    return new Intl.DateTimeFormat(localeTag(), {year:'numeric',month:'short',day:'numeric',weekday:'short'}).format(new Date(y, m - 1, d));
  }
  ...
  async function copyText(content) {
    try {
      ...
      toast(t('toastCopied','복사했습니다.'));
    } catch (_) {
      const ta = document.createElement('textarea'); ta.value = content; ta.style.position = 'fixed'; ta.style.opacity = '0';
      document.body.appendChild(ta); ta.select(); document.execCommand('copy'); ta.remove(); toast(t('toastCopied','복사했습니다.'));
    }
  }
  let toastTimer;
  ...
  function initTheme() {
    ...
    bindEl($('themeBtn'), 'click', () => setTheme(document.documentElement.dataset.theme === 'dark' ? 'light' : 'dark'));
  }

  function initHeader() {
    text('heroDate', new Intl.DateTimeFormat(localeTag(), {year:'numeric',month:'long',day:'numeric',weekday:'long'}).format(new Date()));
    const menuBtn = $('menuBtn');
    bindEl(menuBtn, 'click', () => { menuBtn.setAttribute('aria-expanded','true'); openModal('menuModal'); });
    bindEl($('closeMenu'), 'click', () => { menuBtn?.setAttribute('aria-expanded','false'); closeModal('menuModal'); });
    bindAll($$('#menuModal a'), 'click', () => closeModal('menuModal'));
    $$('.modal').forEach((el) => { if (!el.classList.contains('open')) el.inert = true; });
    bindEl($('heroGuide'), 'click', () => speak('다원 하루설계는 오늘 바꿀 딱 한 가지를 정하고, 약 3분 동안 오늘 한 일과 감정, 내일 첫 행동을 기록하는 생활설계입니다. 완벽하게 이어가는 것보다 쉬어도 다시 시작한 경험을 기록합니다.'));
    ...
    if ('IntersectionObserver' in window) {
      ...
      sections.forEach(id => { const el = $(id); if (el) sectionNavObserver.observe(el); });
    }
  }
  ...
  function oneSuggestions() {
    return window.__dawonI18n?.oneSuggestions || ONE_SUGGESTIONS;
  }
  function currentOneAction() {
    return value('oneCustom').trim() || textValue('oneSuggestion') || oneSuggestions()[value('oneArea')]?.[0] || '';
  }
  function textValue(id) { return $(id)?.textContent?.trim() || ''; }
  function recommendOne() {
    const area = value('oneArea') || '마음';
    const map = oneSuggestions();
    const list = map[area] || map.마음 || ONE_SUGGESTIONS.마음;
  ...
  function recordOne(kind) {
    ...
    if (!action) return toast(t('toastNeedOne', '오늘 할 한 가지를 먼저 적어주세요.'));
    ...
    toast(kind === 'done' ? t('toastOneDone', '오늘 한 가지의 완료 기록을 남겼습니다.') : t('toastOneRestart', '다시 시작한 경험을 기록했습니다.'));
  }
  function renderOne() {
    const items = load(KEYS.one, []);
    ...
    box.replaceChildren();
    if (!items.length) { const e = document.createElement('div'); e.textContent=t('emptyOneHistory','아직 오늘 한 가지 기록이 없습니다.'); box.appendChild(e); return; }
    items.slice(0,5).forEach(item => {
      const div = document.createElement('div');
      const mark = item.kind === 'done' ? '✓ 완료' : '↻ 다시 시작';
      div.innerHTML = `${escapeHtml(dateLabel(item.day))} · ${escapeHtml(mark)} · ${withEmo(item.action)}`; box.appendChild(div);
    });
  }
  ...
  function initOne() {
    bindEl($('oneArea'), 'change', recommendOne);
    bindEl($('oneRecommend'), 'click', recommendOne);
    bindEl($('oneToToday'), 'click', () => { const action=currentOneAction(); if (!action) return toast(t('toastNeedOneShort','오늘 한 가지를 적어주세요.')); setAction(action); toast(t('toastToToday','3분 오늘설계의 실천란에 넣었습니다.')); });
    bindEl($('oneToChallenge'), 'click', () => { const action=currentOneAction(); if (!action) return toast(t('toastNeedPractice','실천할 한 가지를 적어주세요.')); value('challengeTitle',action); showSection('challenge'); toast(t('toastToChallenge','7일 실천 이름에 넣었습니다.')); });
    bindEl($('oneDone'), 'click', () => recordOne('done'));
    bindEl($('oneRestart'), 'click', () => recordOne('restart'));
    recommendOne(); renderOne();
  }

  const MISSION_LABELS = {
    praise:'칭찬', comfort:'위로', thanks:'감사', reflect:'반성', meditate:'명상', pray:'기도',
    exercise:'운동', clean:'청소', cook:'요리', laundry:'세탁', organize:'정리정돈',
    today:'오늘 한 일', tomorrow:'내일 할 일', priority:'우선순위', one:'오늘 하나',
    add:'더하기', subtract:'빼기', direction:'방안', method:'방도', alternative:'대안', idea:'아이디어 제안', communicate:'소통'
  };
  function liveMissionLabels() {
    return window.__dawonI18n?.missionLabels || MISSION_LABELS;
  }
  function liveMissionTitle(key) {
    return liveMissionLabels()[key] || MISSION_LABELS[key] || key;
  }
  const MISSION_AREA = {
    praise:'마음', comfort:'마음', thanks:'마음', reflect:'마음', meditate:'마음', pray:'마음',
    exercise:'건강', clean:'공간', cook:'공간', laundry:'공간', organize:'공간',
    today:'배움', tomorrow:'배움', priority:'배움', one:'마음',
    add:'창작', subtract:'창작', direction:'배움', method:'배움', alternative:'배움', idea:'창작', communicate:'관계'
  };
  let currentMissionKey = null;
  function missionStoreKey(k) { return `dawon_mission_${k}`; }
  function readMission(k) {
    try { return JSON.parse(storageGet(missionStoreKey(k)) || '{}') || {}; } catch { return {}; }
  }
  function missionActionText() {
    return value('missionQ1').trim() || value('missionQ0').trim() || liveMissionTitle(currentMissionKey) || '';
  }
  function markMissionChips() {
    $$('[data-mission]').forEach((btn) => {
      const saved = readMission(btn.dataset.mission);
      const filled = Boolean(saved?.date || saved?.q0 || saved?.q1);
      btn.classList.toggle('has-save', filled);
    });
  }
  function openMission(key) {
    if (!liveMissionTitle(key) && !MISSION_LABELS[key]) return;
    currentMissionKey = key;
    text('missionModalTitle', liveMissionTitle(key));
    const saved = readMission(key);
    for (let i = 0; i < 5; i++) value(`missionQ${i}`, saved[`q${i}`] || '');
    text('missionSaveStatus', saved.date ? `${t('prevSaved','이전 저장')} · ${dateLabel(String(saved.date).slice(0, 10))}` : '');
    openModal('missionModal');
  }
  function persistMission() {
    if (!currentMissionKey) return null;
    const data = { date: new Date().toISOString() };
    for (let i = 0; i < 5; i++) data[`q${i}`] = value(`missionQ${i}`).trim();
    storageSet(missionStoreKey(currentMissionKey), JSON.stringify(data));
    const area = MISSION_AREA[currentMissionKey];
    const action = missionActionText();
    if (area && $('oneArea')) value('oneArea', area);
    if (action && $('oneCustom')) value('oneCustom', action);
    markMissionChips();
    return data;
  }
  function initMissions() {
    if (!$('missionModal') && !$$('[data-mission]').length) return;
    bindAll($$('[data-mission]'), 'click', (ev) => openMission(ev.currentTarget.dataset.mission));
    bindEl($('closeMission'), 'click', () => closeModal('missionModal'));
    bindEl($('missionSave'), 'click', () => {
      if (!persistMission()) return;
      text('missionSaveStatus', window.__dawonI18n?.savedMessage || '저장했습니다. 결과에서 배운 방법을 다른 생활에도 적용해 보세요.');
      toast(window.__dawonI18n?.savedMessage || '생활미션을 저장했습니다.');
    });
    bindEl($('missionToToday'), 'click', () => {
      if (!persistMission()) return;
      const action = missionActionText();
      if (!action) return toast(t('toastMissionNeed','오늘 가장 중요한 하나를 적어주세요.'));
      closeModal('missionModal');
      setAction(action);
      toast(t('toastToToday','3분 오늘설계의 실천란에 넣었습니다.'));
    });
    bindEl($('missionToChallenge'), 'click', () => {
      if (!persistMission()) return;
      const action = missionActionText();
      if (!action) return toast(t('toastMission7','7일로 보낼 행동을 적어주세요.'));
      value('challengeTitle', action);
      closeModal('missionModal');
      showSection('challenge');
      toast(t('toastToChallenge','7일 실천 이름에 넣었습니다.'));
    });
    markMissionChips();
  }

  function applyTransferSampleDefaults(force) {
    const from = $('transferFrom');
    const to = $('transferTo');
    if (!from || !to) return;
    const sampleFrom = t('transferSampleFrom', '정리정돈 원리');
    const sampleTo = t('transferSampleTo', '공부에 적용');
    const knownFrom = ['정리정돈 원리', 'Organizing principle', '整理整頓の原理', '整理整顿原理'];
    const knownTo = ['공부에 적용', 'Apply to study', '勉強に適用', '应用到学习'];
    const curFrom = String(from.value || '').trim();
    const curTo = String(to.value || '').trim();
    if (force || !curFrom || knownFrom.includes(curFrom)) from.value = sampleFrom;
    if (force || !curTo || knownTo.includes(curTo)) to.value = sampleTo;
    try {
      const saved = JSON.parse(storageGet('dawon_transfer') || '{}') || {};
      if (saved.date) text('transferStatus', `${t('recentSaved','최근 저장')} · ${dateLabel(String(saved.date).slice(0, 10))}`);
      else text('transferStatus', t('transferStatusLocal','전이 기록은 이 브라우저에 저장됩니다.'));
    } catch {
      text('transferStatus', t('transferStatusLocal','전이 기록은 이 브라우저에 저장됩니다.'));
    }
  }
  function initTransfer() {
    ...
    try {
      const saved = JSON.parse(storageGet('dawon_transfer') || '{}') || {};
      if (saved.a) value('transferFrom', saved.a);
      if (saved.b) value('transferTo', saved.b);
      if (saved.date) text('transferStatus', `${t('recentSaved','최근 저장')} · ${dateLabel(String(saved.date).slice(0, 10))}`);
    } catch {}
    applyTransferSampleDefaults(false);
    bindAll($$('.transfer-row'), 'click', (ev) => {
      const label = ev.currentTarget.querySelector('b')?.textContent?.trim() || '';
      const detail = ev.currentTarget.querySelector('span')?.textContent?.trim() || '';
      if (label) value('transferFrom', detail ? `${label} — ${detail}` : label);
    });
    bindEl($('transferSave'), 'click', () => {
      const a = value('transferFrom').trim();
      const b = value('transferTo').trim();
      if (!a && !b) return toast(t('toastTransferNeed','전이할 원리나 적용할 곳을 적어주세요.'));
      storageSet('dawon_transfer', JSON.stringify({ a, b, date: new Date().toISOString() }));
      text('transferStatus', t('toastTransferSaved','전이 기록을 저장했습니다.'));
      toast(t('toastTransferSaved','전이 기록을 저장했습니다.'));
    });
    bindEl($('transferToToday'), 'click', () => {
      const a = value('transferFrom').trim();
      const b = value('transferTo').trim();
      const action = b ? `${a || '원리'} → ${b}` : a;
      if (!action) return toast(t('toastTransferContent','전이할 내용을 적어주세요.'));
      storageSet('dawon_transfer', JSON.stringify({ a, b, date: new Date().toISOString() }));
      setAction(action);
      toast(t('toastTransferApplied','오늘설계 실천란에 반영했습니다.'));
    });
    bindEl($('transferToChallenge'), 'click', () => {
      const a = value('transferFrom').trim();
      const b = value('transferTo').trim();
      const title = a && b ? `${a} → ${b}` : (a || b);
      if (!title) return toast(t('toastTransferTitle','실천 이름으로 보낼 내용을 적어주세요.'));
      value('challengeTitle', title);
      if (a) value('challengeReason', `전이 원리: ${a}`);
      toast(t('toastToChallenge','7일 실천 이름에 넣었습니다.'));
    });
  }
  ...
  function initIdeaLab() {
    if (!$('ideaLab')) return;
    const focusMap = { discover:'pPriority', shape:'pObstacle', propose:'pMethod', test:'pAction' };
    bindAll($$('[data-idea-step]'), 'click', (ev) => {
      const step = ev.currentTarget.dataset.ideaStep;
      const targetId = focusMap[step] || 'pPriority';
      ...
      const hintKey = ({ discover:'ideaLabHint1', shape:'ideaLabHint2', propose:'ideaLabHint3', test:'ideaLabHint4' })[step];
      toast(hintKey ? t(hintKey) : t('toastToPrecision','정밀설계로 이동합니다.'));
    });
  }
  ...
  function updateDayPreviewNow() {
    ...
    html('cardTitle', day.action ? `${t('todayOnePrefix','오늘의 한 가지')} · ${withEmo(day.action)}` : t('cardTitleDefault','오늘의 핵심을 확인합니다'));
    html('summary', withEmo(daySummary(day)).replace(/\n/g,'<br>'));
  ...
  function updateDayPreview() {
    if (dayPreviewRaf) cancelAnimationFrame(dayPreviewRaf);
    dayPreviewRaf = requestAnimationFrame(() => { dayPreviewRaf = 0; updateDayPreviewNow(); });
  }
  ...
  function saveDay() {
    ...
    if (!day.done && !day.mood && !day.action && !day.tomorrow && !day.selfWord) return toast(t('toastDayNeed','핵심 항목 가운데 한 가지 이상을 입력해 주세요.'));
    ...
    toast(t('toastDaySaved','오늘설계를 완료했습니다.'));
  }
  function sampleDay() {
    state.mood='감사';
    ...
    toast(t('toastDaySample','3분 예시를 넣었습니다. 내 내용으로 바꿔보세요.'));
  }
  function clearDay() {
  ...
  function speak(content) {
    if (!('speechSynthesis' in window)) return toast(t('toastNoSpeech','이 브라우저는 음성 읽기를 지원하지 않습니다.'));
  ...
  function initSpeechInput() {
    ...
    $$('.mic-btn').forEach((btn) => bindEl(btn, 'click', () => {
      if (!Recognition) return toast(t('toastNeedChrome','Chrome 또는 Edge의 음성입력을 이용해 주세요.'));
      ...
      btn.disabled=true; btn.textContent=t('voiceListening','듣는 중…');
      rec.onresult=e=>{ const heard=e.results?.[0]?.[0]?.transcript||''; flushEmoField(target); target.value = target.value ? `${target.value} ${heard}` : heard; target.dispatchEvent(new Event('input',{bubbles:true})); };
      rec.onerror=()=>toast(t('toastSpeechFail','음성을 인식하지 못했습니다. 다시 시도해 주세요.'));
      rec.onend=()=>{btn.disabled=false;btn.textContent=t('voiceIdle','🎙 음성');}; rec.start();
    }));
  }
  function initDay() {
    bindAll($$('.mood'), 'click', (ev) => { const btn=ev.currentTarget; state.mood=btn.dataset.mood||''; $$('.mood').forEach(x=>x.classList.toggle('active',x===btn)); updateDayPreview(); });
    ['done','action','tomorrow','selfWord','area','energy','result','learn','memo'].forEach(id=>bindEl($(id), 'input', updateDayPreview));
    bindEl($('saveDay'), 'click', saveDay); bindEl($('sampleDay'), 'click', sampleDay); bindEl($('clearDay'), 'click', clearDay);
    bindEl($('copySummary'), 'click', ()=>copyText(daySummary())); bindEl($('speakSummary'), 'click', ()=>speak(daySummary()));
    const today=getDays()[todayKey()]; if(today) fillDay(today); else updateDayPreview();
    initSpeechInput();
  }

  const P_DOMAINS = [
    ['Emotion','domainEmotion'],['Relation','domainRelation'],['Money','domainMoney'],['Health','domainHealth'],['Creation','domainCreation'],['Business','domainBusiness']
  ];
  function domainLabel(key) {
    const row = P_DOMAINS.find(([k]) => k === key);
    return t(row?.[1] || key, row?.[1] || key);
  }
  function collectPrecision() {
    ...
    try {
      const domains={};
      P_DOMAINS.forEach(([key])=>domains[key]={label:domainLabel(key),score:Number(value(`p${key}`)||6),note:value(`p${key}Note`).trim()});
  ...
  function precisionSummary(data=collectPrecision()) {
    const {avg,lowest}=precisionAnalysis(data); const lines=[`${t('avgLabel','평균')} 6 · ${avg.toFixed(1)}/10`,`${t('priorityArea','우선 영역:')} ${lowest.label} ${lowest.score}/10`];
  ...
  function updatePrecisionUINow() {
    const data=collectPrecision(); const {avg,lowest}=precisionAnalysis(data); text('pAverage',avg.toFixed(1));text('pLowest',lowest.label);html('precisionSummaryText', withEmo(precisionSummary(data)).replace(/\n/g,'<br>'));
  ...
  let precisionUiRaf = 0;
  function updatePrecisionUI() {
    if (precisionUiRaf) cancelAnimationFrame(precisionUiRaf);
    precisionUiRaf = requestAnimationFrame(() => { precisionUiRaf = 0; updatePrecisionUINow(); });
  }
  ...
  function savePrecision() {
    ...
    if(!data.priority&&!data.action&&!Object.values(data.domains).some(x=>x.note))return toast(t('toastPrecisionNeed','우선순위나 영역별 메모를 한 가지 이상 적어주세요.'));
    const all=getPrecision();all[data.date]=data;save(KEYS.precision,all);text('precisionStatus',`${dateLabel(data.date)} 정밀설계를 저장했습니다.`);renderPrecisionHistory();renderReports();toast(t('toastPrecisionSaved','정밀설계를 저장했습니다.'));
  }
  function renderPrecisionHistory() {
    ...
    if(!items.length){const p=document.createElement('div');p.className='precision-history-item';p.textContent=t('emptyPrecision','아직 저장된 정밀설계가 없습니다.');box.appendChild(p);return;}
    items.slice(0,5).forEach(item=>{const btn=document.createElement('button');btn.type='button';btn.className='precision-history-item';const a=precisionAnalysis(item);btn.innerHTML=`<b>${escapeHtml(dateLabel(item.date))} · 평균 ${a.avg.toFixed(1)}</b><span>${withEmo(item.priority||`우선 영역: ${a.lowest.label}`)}</span>`;btn.addEventListener('click',()=>{fillPrecision(item);showSection('precision');});box.appendChild(btn);});
  }
  function samplePrecision(){fillPrecision({domains:{Emotion:{score:6,note:'해야 할 일이 많아 마음이 조금 조급합니다.'},Relation:{score:7,note:'가족과 짧게라도 대화할 시간이 필요합니다.'},Money:{score:6,note:'오늘 지출을 확인합니다.'},Health:{score:5,note:'수면이 부족해 오후 에너지가 떨어집니다.'},Creation:{score:8,note:'원고 방향은 분명합니다.'},Business:{score:6,note:'홈페이지 첫 행동을 더 단순하게 만듭니다.'}},priority:'수면과 에너지를 먼저 회복한다.',reason:'몸의 에너지가 있어야 창작과 관계도 이어갈 수 있습니다.',action:'오늘 밤 11시 전에 휴대전화를 내려놓고 잠자리를 준비한다.',obstacle:'늦은 시간까지 자료를 보는 습관',method:'10시 30분에 알림을 설정하고 책상 정리를 시작한다.',alternative:'바로 잠들기 어렵다면 조명을 낮추고 종이책을 10분 읽는다.',tomorrow:'기상 직후 수면 시간을 기록한다.',insight:'할 일을 더 늘리기보다 회복 시간을 먼저 확보해야 합니다.'});toast(t('toastPrecisionSample','정밀설계 예시를 넣었습니다.'));}
  function clearPrecision(){if(!confirm('현재 정밀설계 입력을 비울까요? 저장 기록은 유지됩니다.'))return;fillPrecision({});}
  function initPrecision(){P_DOMAINS.forEach(([key])=>{bindEl($(`p${key}`),'input',updatePrecisionUI);bindEl($(`p${key}Note`),'input',updatePrecisionUI);});['pPriority','pReason','pAction','pObstacle','pMethod','pAlternative','pTomorrow','pInsight'].forEach(id=>bindEl($(id),'input',updatePrecisionUI));bindEl($('savePrecision'),'click',savePrecision);bindEl($('samplePrecision'),'click',samplePrecision);bindEl($('clearPrecision'),'click',clearPrecision);bindEl($('copyPrecision'),'click',()=>copyText(precisionSummary()));bindEl($('exportPrecision'),'click',()=>download(`dawon-precision-${todayKey()}.json`,JSON.stringify(getPrecision(),null,2),'application/json'));bindEl($('precisionToChallenge'),'click',()=>{const a=value('pAction').trim()||value('pPriority').trim();if(!a)return toast(t('toastPrecisionTo7Need','7일 동안 반복할 행동을 먼저 적어주세요.'));value('challengeTitle',a);showSection('challenge');toast(t('toastPrecisionTo7','정밀설계 행동을 7일 실천으로 보냈습니다.'));});const today=getPrecision()[todayKey()];if(today)fillPrecision(today);else updatePrecisionUI();renderPrecisionHistory();}
  ...
  function renderChallenge(){const c=getChallenge();const box=$('challengeDays');if(!box)return;box.replaceChildren();if(!c){text('challengeName',t('challengeEmpty','아직 시작한 실천설계가 없습니다'));text('challengeCount','0/7');text('challengeMessage',t('challengeEmptyMsg','실천 이름을 정하고 첫날을 체크해 보세요.'));$('challengeBar')?.style.setProperty('width','0%');return;}html('challengeName', withEmo(c.title));const done=c.days.filter(x=>x.done).length;text('challengeCount',`${done}/7`);$('challengeBar')?.style.setProperty('width',`${done/7*100}%`);if(done===7){html('challengeMessage',`${t('challengeDoneMsg','7일 실천을 마쳤습니다. 보상:')} ${withEmo(c.reward||t('praiseSelf','나를 충분히 칭찬하기'))}`);}else{text('challengeMessage',done>=5?t('challengeSeedMsg','씨앗과정 성공 기준 5일을 달성했습니다.'):`${7-done}${t('challengeLeftMsg','번의 작은 실천 기회가 남았습니다.')}`);}c.days.forEach((day,index)=>{const btn=document.createElement('button');btn.type='button';btn.className=`day-check${day.done?' checked':''}`;btn.innerHTML=`<b>${day.done?'✓':index+1}</b>${escapeHtml(day.date.slice(5))}`;btn.setAttribute('aria-pressed',String(day.done));btn.addEventListener('click',()=>{const next=getChallenge();next.days[index].done=!next.days[index].done;next.updatedAt=new Date().toISOString();save(KEYS.challenge,next);renderChallenge();renderReports();renderSchool();});box.appendChild(btn);});}
  function initChallenge(){bindEl($('saveChallenge'),'click',()=>{flushEmoFields();const title=value('challengeTitle').trim();if(!title)return toast(t('toastChallengeNeed','7일 동안 반복할 실천 이름을 적어주세요.'));save(KEYS.challenge,createChallenge(title,value('challengeReason').trim(),value('challengeReward').trim()));renderChallenge();renderReports();toast(t('toastChallengeStart','7일 실천설계를 시작했습니다.'));});bindEl($('challengeSample'),'click',()=>{value('challengeTitle','매일 10분 걷기');value('challengeReason','몸과 마음의 에너지를 생활 속에서 회복하기 위해');value('challengeReward','좋아하는 책 한 권 읽기');});bindEl($('resetChallenge'),'click',()=>{if(confirm(t('toastChallengeReset','현재 7일 실천 기록을 초기화할까요?'))){storageRemove(KEYS.challenge);renderChallenge();renderReports();}});bindEl($('firstStart7'),'click',()=>{closeModal('firstCompleteOverlay');const action=value('action').trim();if(action)value('challengeTitle',action);showSection('challenge');});bindEl($('firstExploreLater'),'click',()=>closeModal('firstCompleteOverlay'));renderChallenge();}

  function renderReports(){const days=getDays();const items=Object.values(days).sort((a,b)=>b.date.localeCompare(a.date));const one=load(KEYS.one,[]);const restarts=one.filter(x=>x.kind==='restart').length;const actions=items.filter(x=>x.action).length;const avg=items.length?Math.round(items.reduce((s,x)=>s+dayScore(x),0)/items.length):0;const freq={};items.forEach(x=>{const k=(x.action||'').trim();if(k)freq[k]=(freq[k]||0)+1;});const top=Object.entries(freq).sort((a,b)=>b[1]-a[1])[0]?.[0]||'-';text('metricCount',items.length);text('metricStreak',`${restarts}${t('timesUnit','회')}`);html('metricTopHabit', withEmo(top.length>18?`${top.slice(0,18)}…`:top));text('metricActions',actions);text('metricAvg',avg);text('heroRecords',items.length);text('heroStreak',`${restarts}${t('timesUnit','회')}`);
    const c=getChallenge();const repeat=c?Math.round(c.days.filter(x=>x.done).length/7*100):0;const last30=items.filter(x=>new Date(`${x.date}T00:00:00`)>=addDays(new Date(),-29));const recordRate=Math.round(last30.length/30*100);const actionRate=items.length?Math.round(actions/items.length*100):0;const resultRate=items.length?Math.round(items.filter(x=>x.result).length/items.length*100):0;const restartScore=Math.min(100,restarts*10);const pi=Math.round(recordRate*.3+actionRate*.3+resultRate*.2+repeat*.15+restartScore*.05);text('practiceIndexTotal',pi);text('heroScore',pi);[['piRecord',recordRate],['piAction',actionRate],['piResult',resultRate],['piRepeat',repeat]].forEach(([id,n])=>{text(id,`${n}%`);$(`${id}Bar`)?.style.setProperty('width',`${n}%`);});text('piRestart',`${restarts}${t('timesUnit','회')}`);$('piRestartBar')?.style.setProperty('width',`${restartScore}%`);
    const history=$('history');if(history){history.replaceChildren();if(!items.length){const d=document.createElement('div');d.className='empty';d.textContent=t('emptyHistory','아직 저장된 기록이 없습니다.');history.appendChild(d);}items.slice(0,10).forEach(item=>{const b=document.createElement('button');b.type='button';b.className='history-item';b.innerHTML=`<b>${escapeHtml(dateLabel(item.date))} · ${escapeHtml(item.mood||t('moodUnset','감정 미선택'))}</b><small>${withEmo(item.action||item.done||t('recordCheck','기록 확인'))}</small>`;b.addEventListener('click',()=>{fillDay(item);showSection('today');});history.appendChild(b);});}
    const area={};items.forEach(x=>area[x.area||t('defaultArea','나')]=(area[x.area||t('defaultArea','나')]||0)+1);const max=Math.max(1,...Object.values(area));const bars=$('areaBars');if(bars){bars.replaceChildren();if(!Object.keys(area).length){const e=document.createElement('div');e.className='empty';e.textContent=t('emptyArea','기록이 쌓이면 생활영역 분포가 표시됩니다.');bars.appendChild(e);}Object.entries(area).sort((a,b)=>b[1]-a[1]).forEach(([k,n])=>bars.appendChild(makeBar(k,n/max*100,String(n))));}
    const ps=Object.values(getPrecision()).sort((a,b)=>b.date.localeCompare(a.date));text('precisionReportCount',ps.length);if(ps.length){const a=precisionAnalysis(ps[0]);text('precisionReportAvg',a.avg.toFixed(1));text('precisionReportLowest',a.lowest.label);html('precisionReportPriority', withEmo(ps[0].priority||'-'));}else{text('precisionReportAvg','-');text('precisionReportLowest','-');text('precisionReportPriority','-');}
  }
  function renderSchool(){const dates=Object.keys(getDays()).sort();const now=new Date();[[7,'school7Bar','school7Text',5],[30,'school30Bar','school30Text',20],[90,'school90Bar','school90Text',60],[365,'school365Bar','school365Text',240]].forEach(([span,barId,textId,target])=>{const count=dates.filter(k=>new Date(`${k}T00:00:00`)>=addDays(now,-(span-1))).length;$(barId)?.style.setProperty('width',`${Math.min(100,count/target*100)}%`);text(textId,`${count}${t('schoolDaysRecorded','일 기록')} · ${t('schoolSuccessBase','성공 기준')} ${target}${t('schoolDaysUnit','일')}`);});}
  function initSchool(){bindAll($$('[data-lesson]'),'click',(ev)=>{setAction(ev.currentTarget.dataset.lesson);toast(t('toastLessonLinked','대표 작품의 질문을 오늘 행동에 연결했습니다.'));});bindAll($$('[data-flower]'),'click',(ev)=>{value('selfWord',ev.currentTarget.dataset.flower);updateDayPreview();showSection('today');toast(t('toastFlowerLinked','꽃인물화 질문을 오늘설계에 넣었습니다.'));});[['makeLifeBook',30,'30일 생활책'],['makeGrowthBook',90,'90일 성장책'],['makeLifeAlmanac',365,'365일 생활연감']].forEach(([id,span,title])=>bindEl($(id),'click',()=>makeLifeBook(span,title)));renderSchool();}
  function makeLifeBook(span,title){const items=Object.values(getDays()).sort((a,b)=>a.date.localeCompare(b.date)).slice(-span);if(!items.length)return toast(t('toastNeedDayRecord','먼저 하루 기록을 한 건 이상 저장해 주세요.'));const rows=items.map(x=>`<article><h2>${escapeHtml(dateLabel(x.date))}</h2><p><b>오늘 한 일</b> ${withEmo(x.done||'-')}</p><p><b>감정</b> ${escapeHtml(x.mood||'-')}</p><p><b>오늘 하나</b> ${withEmo(x.action||'-')}</p><p><b>결과</b> ${withEmo(x.result||'-')}</p><p><b>배움</b> ${withEmo(x.learn||'-')}</p><p><b>내일 첫 행동</b> ${withEmo(x.tomorrow||'-')}</p></article>`).join('');const html=`<!doctype html><html lang="ko"><meta charset="utf-8"><title>${escapeHtml(title)}</title><style>body{max-width:820px;margin:40px auto;font-family:system-ui;line-height:1.7;color:#222}header{padding:60px 30px;background:#143b31;color:#fff;border-radius:24px}article{padding:25px 5px;border-bottom:1px solid #ddd}h1{font-size:42px}h2{font-size:21px}b{color:#0c8877}@media print{body{margin:0}article{break-inside:avoid}}</style><header><small>DAWON LIFE DESIGN BOOK</small><h1>${escapeHtml(title)}</h1><p>${items.length}일의 기록으로 만든 현재까지의 생활책</p></header>${rows}<script>window.onload=()=>window.print()<\/script></html>`;download(`다원_${title}_${todayKey()}.html`,html,'text/html;charset=utf-8');toast(`${title} 파일을 만들었습니다.`);}
  ...
  function setMember(member){state.member=member||null;const logged=Boolean(member);text('accountName',logged?(member.name||member.email||t('guestName','회원')):t('guestName','게스트'));text('accountBtn',logged?t('myAccount','내 계정'):t('login','로그인'));$('cloudDot')?.classList.toggle('online',logged&&state.apiOnline);$('authGuestPanels')?.toggleAttribute('hidden',logged);$('forgotPanel')?.toggleAttribute('hidden',true);$('resetPanel')?.toggleAttribute('hidden',true);$('memberPanel')?.toggleAttribute('hidden',!logged);if(logged){text('memberName',member.name||t('guestName','회원'));text('memberEmail',member.email||'');value('checkoutName',member.name||'');value('checkoutEmail',member.email||'');}window.dawonSetAccessState?.({authenticated:logged,active:Boolean(member?.subscription?.active),planName:member?.subscription?.planName||t('planFree','무료'),endsAt:member?.subscription?.endsAt||null});}
  ...
  function initAuth(){const open=()=>{openModal('authModal');state.member?null:authPanels('loginPanel');};$('closeAuth')?.addEventListener('click',()=>closeModal('authModal'));$('showLoginPanel')?.addEventListener('click',()=>authPanels('loginPanel'));$('showRegisterPanel')?.addEventListener('click',()=>authPanels('registerPanel'));$('showForgotPanel')?.addEventListener('click',()=>authPanels('forgotPanel'));
    ...
    $('logoutBtn')?.addEventListener('click',async()=>{try{if(CONFIG.apiBase)await apiFetch('/auth/logout',{method:'POST',body:'{}'});}catch(_){ }setMember(null);closeModal('authModal');toast(t('toastLoggedOut','로그아웃했습니다.'));});
    ...
    $('deleteAccountBtn')?.addEventListener('click',async()=>{const password=value('deletePassword');if(!password)return authMessage(t('loginErrPassword','현재 비밀번호를 입력해 주세요.'),true);if(value('deleteConfirmText').trim()!==t('authDeleteConfirmPhrase','탈퇴합니다'))return authMessage(t('authDeleteConfirmPhrase','탈퇴합니다'),true);if(!confirm(t('toastChallengeReset','회원정보와 서버 기록을 삭제할까요?')))return;try{await apiFetch('/auth/account',{method:'DELETE',body:JSON.stringify({password})});setMember(null);closeModal('authModal');toast(t('toastDeleted','회원탈퇴 처리가 완료되었습니다.'));}catch(e){authMessage(e.message,true);}});
  ...
  function initQuickTools(){bindAll($$('.quick-tools [data-create]'),'click',(ev)=>{const btn=ev.currentTarget;const day=collectDay();value('ideaTitle',day.action||'오늘의 생활설계');value('ideaSource',daySummary(day));const target=$('typeGrid')?.querySelector(`[data-type="${btn.dataset.create}"]`);target?.click();showSection('studio');setTimeout(()=>$('generateContent')?.click(),150);toast(t('toastStudioLinked','오늘 기록을 창작스튜디오에 연결했습니다.'));});}
  function renderAll(){renderOne();renderChallenge();renderPrecisionHistory();renderReports();renderSchool();updateDayPreviewNow();updatePrecisionUINow();applyTransferSampleDefaults(false);if($('heroDate'))text('heroDate', new Intl.DateTimeFormat(localeTag(), {year:'numeric',month:'long',day:'numeric',weekday:'long'}).format(new Date()));window.dispatchEvent(new CustomEvent('dawon-data-changed'));}

  function bindDawonOsPage() {
    beginPageBindings();
    if (!window.__dawonOsCoreOnce) {
      window.__dawonOsCoreOnce = true;
    ...
    initTheme(); initHeader(); initExports(); initAuth();
    initOne(); initMissions(); initDay(); initPrecision(); initIdeaLab(); initChallenge(); initTransfer(); initSchool(); initQuickTools();
    renderReports();
  }

  window.Dawon = { $, $$, CONFIG, KEYS, state, load, save, storageGet, storageSet, storageRemove, escapeHtml, todayKey, dateLabel, localeTag, addDays, download, copyText, toast, openModal, closeModal, showSection, getDays, setDays, getPrecision, getChallenge, getCurrentDay, collectDay, daySummary, setAction, renderAll, applyTransferSampleDefaults, apiFetch, setMember, checkSession, speak, makeBar, flushEmoField, flushEmoFields, withEmo, html, text, bindPage: bindDawonOsPage, bindFeatures: null };
...
(() => {
  ...
  const flushEmoField = D.flushEmoField || function(el) {
    if (el instanceof HTMLInputElement || el instanceof HTMLTextAreaElement) {
      window.__dawonFlushEmoticonField?.(el);
    }
  };
  const flushEmoFields = D.flushEmoFields || function() {
    window.__dawonFlushEmoticons?.();
  };
  const t = (key, fallback) => {
    const v = window.__dawonI18n?.t?.(key, fallback);
  ...
  const ACCESS_DEFAULT = {authenticated:false,active:false,planName:'Free',endsAt:null};
  ...
  let featureAbortSignal = null;
  function beginFeatureBindings() {
    window.__dawonOsFeaturesAbort?.abort();
    const ac = new AbortController();
    window.__dawonOsFeaturesAbort = ac;
    featureAbortSignal = ac.signal;
  }
  function bindFeat(el, type, fn) {
    if (!el || !featureAbortSignal) return;
    el.addEventListener(type, fn, { signal: featureAbortSignal });
  }
  function bindFeatAll(nodes, type, fn) {
    nodes.forEach((el) => bindFeat(el, type, fn));
  }
  ...
  const stripEmo = (s) => String(s ?? '').replace(/\[\[emo:?\d{2}\]\]/g, ' ').replace(/\s+/g, ' ').trim();
  const slug = (s) => (stripEmo(s).replace(/[\\/:*?"<>|\[\]]/g,'').replace(/\s+/g,'_') || 'dawon');
  ...
  function renderAccess(){const access=window.DAWON_ACCESS_STATE;const active=Boolean(access.active);textSafe('libraryAccessPill',active?t('accessFull','전체 이용 가능'):t('accessPreview','무료 미리보기'));textSafe('libraryAccessTitle',active?`${localizePlanName(access.planName)} · PDF 153`:t('libraryAccessGuestTitle','전자책·만화·오디오북 미리보기 · 첫 가입 7일 무료'));textSafe('libraryAccessDetail',active?(access.endsAt?`${t('accessEnds','이용 만료:')} ${new Date(access.endsAt).toLocaleDateString(localeTag())}`:t('accessActive','활성 이용권이 확인되었습니다.')):t('libraryAccessGuestDetail','미리보기는 누구나 이용할 수 있습니다.'));textSafe('libraryAccessCta',active?t('searchWorks','작품 검색하기'):t('viewPlans','이용권 보기'));const cta=$('libraryAccessCta');if(cta){cta.href=active?'/library':'/subscribe';}renderWorks();renderSubscription();}
  function textSafe(id,v){const el=$(id);if(el)el.textContent=v;}
  function htmlSafe(id,v){const el=$(id);if(el)el.innerHTML=v??'';}
  function withEmoSafe(value=''){return escapeHtml(value).replace(/\[\[emo:?(\d{2})\]\]/g,'<img class="dawon-emo" src="/emoticons/$1.png" alt="">');}
  ...
  function workCard(item) {
    ...
    openBtn.addEventListener('click',()=>{
      if(!allowed){showSection('subscription');return toast(t('toastNeedPassFull','대표 3작품 외 전체 이용은 활성 이용권이 필요합니다.'));}
      if(!item.pdfUrl)return toast(t('toastNeedPdf','이 작품의 PDF 경로를 먼저 등록해 주세요.'));
      if(status==='missing')return toast(t('toastFileMissing','서버에서 파일을 찾지 못했습니다. 등록 경로를 확인해 주세요.'));
    ...
    if(item.format==='audio'){
      const listen=document.createElement('button');listen.type='button';listen.textContent=item.audioUrl?'음원 재생':'실시간 낭독';listen.addEventListener('click',()=>{
        if(!allowed)return toast(t('toastNeedPass','활성 이용권이 필요합니다.'));
  ...
  function valueOf(id,fallback=''){const el=$(id);if(!el)return fallback;if(el instanceof HTMLInputElement||el instanceof HTMLTextAreaElement){window.__dawonFlushEmoticonField?.(el);}return el.value??fallback;}
  function valueSet(id,v){const el=$(id);if(!el)return;el.value=v??'';el.dispatchEvent(new Event('input',{bubbles:true}));}
  function renderStudioOutput(out){studioState.output=out;htmlSafe('contentOutput', withEmoSafe(String(out||'')).replace(/\n/g,'<br>'));}
  ...
  function initWorks(){const no=$('workAdminNo');if(no){no.replaceChildren(...Array.from({length:51},(_,i)=>{const o=document.createElement('option');o.value=i;o.textContent=`BOOK ${pad(i)}`;return o;}));}bindFeatAll($$('.works-tab'),'click',(ev)=>{worksState.format=ev.currentTarget.dataset.workFormat;worksState.page=1;renderWorks();});bindFeat($('worksSearch'),'input',(e)=>{worksState.query=e.target.value;worksState.page=1;renderWorks();});bindFeat($('worksPrev'),'click',()=>{worksState.page--;renderWorks();});bindFeat($('worksNext'),'click',()=>{worksState.page++;renderWorks();});bindFeat($('worksCheckAll'),'click',auditAll);bindFeat($('worksMissingExport'),'click',exportMissing);bindFeat($('workAdminFormat'),'change',()=>{worksState.format=valueOf('workAdminFormat');loadAdmin();renderWorks();});bindFeat($('workAdminNo'),'change',loadAdmin);bindFeat($('worksLoadCurrent'),'click',loadAdmin);bindFeat($('workAdminSave'),'click',saveAdmin);bindFeat($('workAdminCheckOne'),'click',async()=>{const item=getCatalog()[valueOf('workAdminFormat')][Number(valueOf('workAdminNo'))];await auditItem(item,'pdf');renderWorks();});bindFeat($('workAdminClear'),'click',resetAdmin);bindFeat($('workCatalogExport'),'click',()=>download(`dawon-work-catalog-${todayKey()}.json`,JSON.stringify(getCatalog(),null,2),'application/json'));if(!window.__dawonWorksListeners){window.__dawonWorksListeners=true;window.addEventListener('dawon-subscription-changed',renderAccess);window.addEventListener('dawon-data-changed',renderWorks);}loadAdmin();renderAccess();}
  ...
  function generateStudio(){const {title,source}=studioSource(),sent=sourceSentences(source);let out='';const core=sent[0]||source;switch(studioState.type){
      ...
      case 'video': studioState.story=[title,sent[0]||'생각만으로 달라지지 않았다',sent[1]||'오늘의 사실을 확인합니다',sent[2]||'감정은 잘못이 아니라 정보입니다',sent[3]||'10분 안에 할 한 가지를 정합니다',sent[4]||'작은 행동은 결과를 남깁니다',sent[5]||'경험은 나만의 답을 만듭니다'].slice(0,7);out=`[로컬 초안 · 7장면 영상]\n제목｜${title}\n\n${studioState.story.map((x,i)=>`장면 ${i+1}｜${x}`).join('\n')}\n\n마지막 CTA｜dawon84.com · 오늘 한 가지 시작하기`;break;
  ...
  function initStudio(){bindFeatAll($$('.studio-tab'),'click',(ev)=>openStudioTab(ev.currentTarget.dataset.page));bindFeatAll($$('.type-card'),'click',(ev)=>{studioState.type=ev.currentTarget.dataset.type;$$('.type-card').forEach(x=>x.classList.toggle('active',x===ev.currentTarget));});bindFeat($('generateContent'),'click',generateStudio);bindFeat($('useToday'),'click',()=>{const day=getCurrentDay();valueSet('ideaTitle',day.action||'오늘의 생활설계');valueSet('ideaSource',daySummary(day));toast('오늘 기록을 불러왔습니다.');});bindFeat($('contentSample'),'click',()=>{valueSet('ideaTitle','정보가 생활을 바꾸는 순간');valueSet('ideaSource','좋은 정보를 읽고도 행동하지 않으면 생활은 크게 달라지지 않습니다. 정보 하나를 오늘 행동 하나로 번역하고, 결과를 기록해야 나의 기준이 됩니다.');generateStudio();});bindFeat($('copyContent'),'click',()=>copyText(studioState.output||textOf('contentOutput')));bindFeat($('downloadContent'),'click',()=>download(`dawon-content-${todayKey()}.txt`,studioState.output||textOf('contentOutput')));bindFeat($('openVideoStudioFromTransform'),'click',()=>{const {title,source}=studioSource();if(!studioState.story.length)generateStudio();valueSet('dvsTitle',title);const story=studioState.story.length?studioState.story:sourceSentences(source).slice(0,7);if(story.length)valueSet('dvsStory',story.join('\n'));openStudioTab('video');setTimeout(()=>drawVideoFrame(0,0),80);});$('openVideoStudioFromTransform')?.toggleAttribute('disabled',false);bindFeatAll($$('[data-open-video-studio]'),'click',(e)=>{e.preventDefault();openStudioTab('video');});}
  ...
  const emoImgCache={};
  function emoCanvasImage(id){
    if(emoImgCache[id])return emoImgCache[id];
    const img=new Image();
    img.decoding='async';
    img.src=`/emoticons/${id}.png`;
    img.onload=()=>{if(!videoState.rendering&&!videoState.previewing)drawVideoFrame(0,0);};
    emoImgCache[id]=img;
    return img;
  }
  function parseRichText(content){
    const parts=[];const str=String(content||'');const re=/\[\[emo:?(\d{2})\]\]/g;let last=0,m;
    while((m=re.exec(str))){if(m.index>last)parts.push({t:'text',v:str.slice(last,m.index)});parts.push({t:'emo',id:m[1]});last=m.index+m[0].length;}
    if(last<str.length)parts.push({t:'text',v:str.slice(last)});
    return parts.length?parts:[{t:'text',v:str}];
  }
  function measureRich(ctx,parts,emoSize){let w=0;for(const p of parts){w+=p.t==='emo'?emoSize+6:ctx.measureText(p.v).width;}return w;}
  function wrapRichLines(ctx,content,maxWidth,emoSize,maxLines){
    const units=[];
    parseRichText(content).forEach(p=>{
      if(p.t==='emo')units.push(p);
      else String(p.v).split(/(\s+)/).filter(Boolean).forEach(v=>units.push({t:'text',v}));
    });
    const lines=[[]];let width=0;
    for(const u of units){
      const uw=u.t==='emo'?emoSize+6:ctx.measureText(u.v).width;
      const isSpace=u.t==='text'&&/^\s+$/.test(u.v);
      if(width+uw>maxWidth&&lines[lines.length-1].length&&!isSpace){lines.push([]);width=0;}
      if(isSpace&&!width)continue;
      lines[lines.length-1].push(u);width+=uw;
    }
    return lines.filter(l=>l.length).slice(0,maxLines);
  }
  function wrapCanvasText(ctx,content,x,y,maxWidth,lineHeight,maxLines=3,align='left'){
    const emoSize=Math.round(lineHeight*0.92);
    const lines=wrapRichLines(ctx,content,maxWidth,emoSize,maxLines);
    const prevAlign=ctx.textAlign;
    ctx.textAlign='left';
    lines.forEach((parts,i)=>{
      const ly=y+i*lineHeight;
      const total=measureRich(ctx,parts,emoSize);
  ...
  function drawVideoFrame(slideIndex=0,progress=0){const canvas=$('dvsCanvas');if(!canvas)return;const ctx=canvas.getContext('2d'),w=canvas.width,h=canvas.height;ctx.clearRect(0,0,w,h);ctx.fillStyle='#0a251f';ctx.fillRect(0,0,w,h);const slide=videoState.slides[slideIndex%Math.max(1,videoState.slides.length)];if(slide?.image)coverDraw(ctx,slide.image,w,h,progress,valueOf('dvsMotion','cinema'));else{const g=ctx.createLinearGradient(0,0,w,h);g.addColorStop(0,'#0c332b');g.addColorStop(1,'#b7953f');ctx.fillStyle=g;ctx.fillRect(0,0,w,h);}
    ...
    const title=valueOf('dvsTitle').trim()||'다원 하루설계';const lines=captions();const caption=lines[slideIndex%Math.max(1,lines.length)]||slide?.label||'오늘을 확인하고, 내일을 설계하세요';ctx.textAlign='left';ctx.fillStyle='#f2d985';ctx.font=`800 ${Math.max(20,w*.028)}px sans-serif`;ctx.fillText('DAWON LIFE DESIGN',w*.07,h*.09);ctx.fillStyle='#fff';ctx.font=`800 ${Math.max(34,w*.054)}px sans-serif`;wrapCanvasText(ctx,title,w*.07,h*.17,w*.86,Math.max(44,w*.066),2,'left');
    if(checkedValue('dvsCaptions')){ctx.fillStyle='rgba(0,0,0,.55)';roundRect(ctx,w*.055,h*.70,w*.89,h*.17,w*.025);ctx.fill();ctx.fillStyle='#fff';ctx.textAlign='center';ctx.font=`700 ${Math.max(30,w*.045)}px sans-serif`;wrapCanvasText(ctx,caption,w*.5,h*.765,w*.78,Math.max(42,w*.057),3,'center');}
  ...
  function initVideo(){textSafe('dvsCapability',window.MediaRecorder&&HTMLCanvasElement.prototype.captureStream?'실제 영상 생성 지원':'미리보기만 지원');textSafe('dvsFormatChip',supportedMime().includes('mp4')?'MP4 직접 저장 가능':'WebM 저장 · MP4 서버 변환 권장');bindFeat($('dvsMedia'),'change',(e)=>handleMedia(Array.from(e.target.files||[])));bindFeatAll($$('#dvsDuration button'),'click',(ev)=>{videoState.duration=Number(ev.currentTarget.dataset.seconds)||30;$$('#dvsDuration button').forEach(x=>x.classList.toggle('active',x===ev.currentTarget));updateVideoCanvasSize();});bindFeat($('dvsRatio'),'change',updateVideoCanvasSize);bindFeat($('dvsMotion'),'change',()=>drawVideoFrame(0,0));['dvsTitle','dvsStory','dvsCaptions','dvsBrand'].forEach(id=>bindFeat($(id),'input',()=>drawVideoFrame(0,0)));bindFeat($('dvsPreview'),'click',previewVideo);bindFeat($('dvsRender'),'click',renderVideo);bindFeat($('dvsSave'),'click',saveVideo);bindFeat($('dvsNew'),'click',resetVideo);bindFeat($('dvsUseStory'),'click',()=>{const {title,source}=studioSource();if(!studioState.story.length)generateStudio();valueSet('dvsTitle',title||valueOf('dvsTitle'));valueSet('dvsStory',(studioState.story.length?studioState.story:sourceSentences(source).slice(0,7)).join('\n'));drawVideoFrame(0,0);});bindFeat($('dvsUseToday'),'click',()=>{const day=getCurrentDay();valueSet('dvsTitle',day.action||'오늘의 생활설계');valueSet('dvsStory',[day.done||'오늘의 사실을 확인합니다',day.mood?`지금 감정은 ${day.mood}`:'감정을 알아차립니다',day.action||'오늘 하나를 정합니다',day.result||'작은 행동을 실천합니다',day.learn||'결과에서 배웁니다',day.tomorrow||'내일 첫 행동을 정합니다',day.selfWord||'나는 내 생활의 설계자'].join('\n'));drawVideoFrame(0,0);});updateVideoCanvasSize();window.DawonVideo={setExternalSlides,open:()=>openStudioTab('video')};}
  ...
  function initMotion(){bindFeat($('motionComicClose'),'click',()=>{pauseMotion();closeModal('motionComicModal');});bindFeat($('motionPlay'),'click',()=>motion.playing?pauseMotion():playMotion());bindFeat($('motionPrev'),'click',()=>{pauseMotion();if(motion.slides.length){motion.index=(motion.index-1+motion.slides.length)%motion.slides.length;renderMotion();}});bindFeat($('motionNext'),'click',()=>{pauseMotion();if(motion.slides.length){motion.index=(motion.index+1)%motion.slides.length;renderMotion();}});bindFeat($('motionNarration'),'click',()=>{motion.narration=!motion.narration;textSafe('motionNarration',motion.narration?'🔊 낭독 켜짐':'🔇 낭독 꺼짐');});bindFeat($('motionFullscreen'),'click',()=>$('motionComicShell')?.requestFullscreen?.());bindFeat($('motionMakeMovie'),'click',recordMotion);bindFeat($('motionSaveMovie'),'click',()=>{if(!motion.blob)return;const ext=motion.blob.type.includes('mp4')?'mp4':'webm';download(`${slug(motion.item?.title||'dawon-comic')}.${ext}`,motion.blob,motion.blob.type);});bindFeat($('motionBackLive'),'click',()=>{const v=$('motionComicVideo');v?.pause();v?.classList.remove('active');$('motionComicCanvas')?.classList.add('active');renderMotion();});bindFeat($('motionToStudio'),'click',()=>{if(!motion.slides.length)return toast('먼저 PDF 장면을 준비해 주세요.');pauseMotion();closeModal('motionComicModal');setExternalSlides(motion.slides,motion.item?.title||'다원 만화영화',motion.slides.map((_,i)=>`${motion.item?.title||'다원 만화'} · 장면 ${i+1}`));});}

  /* ------------------------------------------------------------------
     Book and proposal builders
  ------------------------------------------------------------------- */
  function bookUpdate(){textSafe('bpTitle',valueOf('bookTitle').trim()||'도서명을 입력하세요');textSafe('bpSubtitle',valueOf('bookSubtitle').trim()||'부제를 입력하면 표지에 반영됩니다.');textSafe('bpAuthor',valueOf('bookAuthor').trim()||'다원작가');htmlSafe('bpBody', withEmoSafe(valueOf('bookManuscript').trim()||'원고를 입력하면 이곳에 표시됩니다.').replace(/\n/g,'<br>'));}
  function standaloneBook(){const title=valueOf('bookTitle').trim()||'다원 생활설계 전자책',subtitle=valueOf('bookSubtitle').trim(),author=valueOf('bookAuthor').trim()||'다원작가',body=withEmoSafe(valueOf('bookManuscript')).replace(/\n/g,'<br>');return`<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(title)}</title><style>body{margin:0;font-family:system-ui;line-height:1.8;color:#202622}.cover{min-height:90vh;padding:8vw;background:#143b31;color:#fff;display:flex;flex-direction:column;justify-content:flex-end}.cover small{color:#e8c76a}.cover h1{font-size:clamp(44px,8vw,82px)}main{max-width:780px;margin:auto;padding:70px 28px;font-size:18px}@media print{.cover{min-height:250mm;break-after:page}}</style></head><body><section class="cover"><small>DAWON LIFE DESIGN BOOK</small><h1>${escapeHtml(title)}</h1><p>${escapeHtml(subtitle)}</p><b>${escapeHtml(author)}</b></section><main>${body}</main></body></html>`;}
  function proposalUpdate(){textSafe('ppTitle',valueOf('proposalTitle').trim()||'자료를 입력하면 제안서가 완성됩니다');textSafe('ppCompany',valueOf('company').trim()||'제안 기업·기관');htmlSafe('ppProblem', withEmoSafe([valueOf('target').trim(),valueOf('problem').trim()].filter(Boolean).join('\n')||'핵심 고객과 생활 문제를 입력하세요.').replace(/\n/g,'<br>'));htmlSafe('ppSolution', withEmoSafe(valueOf('solution').trim()||'해결 방법을 입력하세요.'));const ul=$('ppDeliverables');if(ul){ul.replaceChildren();const lines=valueOf('deliverables').split('\n').map(x=>x.trim()).filter(Boolean);(lines.length?lines:['검색형 기사·영상·실천 프로그램']).forEach(x=>{const li=document.createElement('li');li.innerHTML=withEmoSafe(x);ul.appendChild(li);});}textSafe('ppBudget',valueOf('budget').trim()||'기간·예산 협의');textSafe('ppContact',valueOf('contact').trim()||'연락처 입력 대기');}
  ...
  function initBuilders(){['bookTitle','bookSubtitle','bookAuthor','bookManuscript'].forEach(id=>bindFeat($(id),'input',bookUpdate));bindFeat($('bookSample'),'click',()=>{valueSet('bookTitle','나는 내 생활의 설계자');valueSet('bookSubtitle','확인하고 실천하며 다음을 만드는 기록법');valueSet('bookAuthor','다원작가');valueSet('bookManuscript','# 1장 오늘의 나를 확인하다\n\n생활설계는 거창한 계획이 아닙니다. 오늘 실제로 한 일과 감정을 확인하고, 지금 가능한 행동 하나를 정하는 일입니다.\n\n# 2장 행동은 결과를 남긴다\n\n작은 실천 뒤에 무엇이 달라졌는지 기록하면 경험이 나만의 답이 됩니다.');bookUpdate();});bindFeat($('bookHtml'),'click',()=>download(`${slug(valueOf('bookTitle')||'dawon-book')}.html`,standaloneBook(),'text/html;charset=utf-8'));bindFeat($('bookPrint'),'click',()=>{bookUpdate();const w=window.open('','_blank');if(w){w.document.write(standaloneBook());w.document.close();setTimeout(()=>{try{w.focus();w.print();}catch(_){toast(t('toastPrintPdf','브라우저의 인쇄 기능을 이용해 PDF로 저장해 주세요.'));}},350);}});bindFeat($('bookTxt'),'click',()=>download(`${slug(valueOf('bookTitle')||'dawon-manuscript')}.txt`,`${valueOf('bookTitle')}\n${valueOf('bookSubtitle')}\n저자 ${valueOf('bookAuthor')}\n\n${valueOf('bookManuscript')}`));['company','proposalTitle','target','problem','solution','deliverables','budget','contact'].forEach(id=>bindFeat($(id),'input',proposalUpdate));bindFeat($('proposalSample'),'click',()=>{valueSet('company','○○도서관');valueSet('proposalTitle','다원 30일 생활설계 프로그램 협업 제안');valueSet('target','중장년 이용자와 자기관리 관심 지역주민');valueSet('problem','정보와 강의는 많지만 생활 속 실천과 결과 기록으로 이어지는 구조가 부족합니다.');valueSet('solution','3분 오늘설계와 7일 실천, 성장리포트를 도서관 프로그램에 연결합니다.');valueSet('deliverables','참여자용 3분 오늘설계\n7일 실천카드\n전자책·오디오북 연계 수업\n30일 성장리포트\n성과 공유회 자료');valueSet('budget','4주 / 예산 협의');valueSet('contact','다원 · book8453@naver.com');proposalUpdate();});bindFeat($('proposalHtml'),'click',()=>download(`${slug(valueOf('proposalTitle')||'dawon-proposal')}.html`,proposalHtml(),'text/html;charset=utf-8'));bindFeat($('proposalPrint'),'click',()=>{proposalUpdate();const w=window.open('','_blank');if(w){w.document.write(proposalHtml());w.document.close();setTimeout(()=>{try{w.focus();w.print();}catch(_){toast(t('toastPrintPdf','브라우저의 인쇄 기능을 이용해 PDF로 저장해 주세요.'));}},350);}});bookUpdate();proposalUpdate();}
  ...
  function localizePlanName(raw){
    const name=String(raw||'').trim();
    if(/^subLabel/.test(name)) return t(name, name);
    const free=['','무료','무료 이용','Free','Free access','無料','無料利用','免费','免费使用','subLabelFree'];
    const paid=['유료','Paid','有料','付费'];
    if(free.includes(name)) return t('planFree','무료');
    if(paid.includes(name)) return t('planPaid','유료');
    return name||t('planFree','무료');
  }
  function renderSubscription(){const a=window.DAWON_ACCESS_STATE;const planLabel=localizePlanName(a.planName);textSafe('subStatus',a.active?t('subStatusActive','이용 중'):a.authenticated?t('subStatusFreeMember','무료 회원'):t('subStatusNeedLogin','로그인 필요'));textSafe('subPlan',planLabel);textSafe('subEndsAt',a.endsAt?new Date(a.endsAt).toLocaleDateString(localeTag()):'-');textSafe('subAutoRenew',t('subAutoRenewNone','없음'));textSafe('subscriptionSummary',a.active?t('subSummaryActive','{plan} pass is active. No auto-renewal.').replace('{plan}',planLabel||t('planPaid','유료')):a.authenticated?t('subSummaryFree','현재 무료 회원입니다. 필요한 기간형 이용권을 선택할 수 있습니다.'):t('subMyDesc','로그인하면 현재 이용권과 만료일, 최근 결제 상태를 확인할 수 있습니다.'));textSafe('paymentReadiness',t('paymentReadiness','토스페이먼츠 안전결제 · /subscribe'));}
  async function refreshSubscription(){if(!CONFIG.apiBase)return toast(t('toastSubServer','이용권 서버가 아직 연결되지 않았습니다.'));try{const d=await apiFetch('/billing/subscription',{method:'GET',headers:{}});window.dawonSetAccessState({authenticated:true,active:Boolean(d.active),planName:d.planName||t('planFree','무료'),endsAt:d.endsAt||null});textSafe('subLastPayment',d.lastPayment||'-');textSafe('subRefundState',d.refundState||'-');renderOrders(d.orders||[]);toast(t('toastSubRefreshed','이용권 상태를 새로고침했습니다.'));}catch(e){toast(e.message);}}
  function renderOrders(orders){const box=$('subscriptionOrderList');if(!box)return;box.replaceChildren();if(!orders.length){const e=document.createElement('div');e.className='empty';e.textContent=t('subOrdersEmpty','최근 결제내역이 없습니다.');box.appendChild(e);return;}orders.slice(0,5).forEach(o=>{const d=document.createElement('div');d.className='readiness-item';d.innerHTML=`<i>₩</i><span>${escapeHtml(o.planName||o.orderName||t('planPaid','이용권'))}<br><small>${escapeHtml(o.paidAt||o.createdAt||'')}</small></span><strong>${Number(o.amount||0).toLocaleString(localeTag())}</strong>`;box.appendChild(d);});}
  ...
  function initSubscription(){bindFeatAll($$('.demo-plan'), 'click', (ev) => planOpen(ev.currentTarget.dataset.plan));bindFeat($('closeCheckout'), 'click', ()=>closeModal('checkoutModal'));bindFeat($('checkoutCancel'), 'click', ()=>closeModal('checkoutModal'));['agreeTerms','agreePrivacy','agreeRefund','agreeCharge'].forEach(id=>bindFeat($(id), 'change', checkoutGuard));bindFeat($('checkoutConfirm'), 'click', checkout);bindFeat($('refreshSubscription'), 'click', refreshSubscription);bindFeat($('refundLatest'), 'click', refundLatest);renderSubscription();}

  /* ------------------------------------------------------------------
     Policy modal and integrity helpers
  ------------------------------------------------------------------- */
  function initPolicies(){bindFeatAll($$('[data-policy]'),'click',(e)=>{e.preventDefault();const type=e.currentTarget.dataset.policy;$$('.policy-content').forEach(x=>x.hidden=x.dataset.policyContent!==type);openModal('policyModal');});bindFeat($('closePolicy'),'click',()=>closeModal('policyModal'));}
  ...
  function bindDawonOsFeatures() {
    beginFeatureBindings();
    if (!window.__dawonOsFeaturesOnce) {
      window.__dawonOsFeaturesOnce = true;
      initPolicies(); initIntegrity();
      if (!window.__dawonSubListener) {
        window.__dawonSubListener = true;
        window.addEventListener('dawon-subscription-changed', renderAccess);
        window.addEventListener('dawon-lang-changed', () => { renderAccess(); renderSubscription(); });
      }
    }
    initSubscription(); initWorks(); initStudio(); initVideo(); initMotion(); initBuilders();
    renderAccess();
  }
  if (window.Dawon) window.Dawon.bindFeatures = bindDawonOsFeatures;
  bindDawonOsFeatures();
})();
